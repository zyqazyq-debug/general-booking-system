...
async function fixture() {
  ...
  const egressComposeFile = join(directory, 'compose.preprod-telegram-egress.yml');
  await writeFile(composeFile, 'services: {}\n');
  await writeFile(egressComposeFile, 'services: {}\n');
  return { releaseRoot, composeFile, egressComposeFile };
}
...
test('blue activation stops and proves the old worker before starting one healthy manifest-bound worker', async () => {
  ...
  try {
    const result = await runSingletonAction({ ...ARGS, 'compose-file': value.composeFile, 'egress-compose-file': value.egressComposeFile }, runtime);
...
test('a residual old worker after stop fails closed before target activation', async () => {
  ...
  try {
    await assert.rejects(runSingletonAction({ ...ARGS, 'compose-file': value.composeFile, 'egress-compose-file': value.egressComposeFile }, runtime), /remains running after stop/);
...
test('two running workers are rejected without stopping or starting either', async () => {
  ...
  try {
    await assert.rejects(runSingletonAction({ ...ARGS, 'compose-file': value.composeFile, 'egress-compose-file': value.egressComposeFile }, runtime), /two singleton workers/);
...
test('receipt replay readback never executes stop or compose up', async () => {
  ...
  try {
    const result = await runSingletonAction({ ...ARGS, 'compose-file': value.composeFile, 'egress-compose-file': value.egressComposeFile, readback: 'true' }, runtime);
    assert.equal(result.workerHealth, 'healthy');
    assert.equal(runtime.calls.some((argv) => argv[0] === 'container' && argv[1] === 'stop'), false);
    assert.equal(runtime.calls.some((argv) => argv.includes('up')), false);
  } finally { await rm(value.releaseRoot, { recursive: true, force: true }); }
...
test('first transfer permits the missing outbox flag only for the exact observed legacy green source', async () => {
  ...
  const legacyArgs = { ...ARGS, 'source-release-id': LEGACY_OLD_BINDING.releaseId, 'source-git-sha': LEGACY_OLD_BINDING.gitSha,
    ...
    'source-backend-digest': LEGACY_OLD_BINDING.imageId, 'compose-file': value.composeFile, 'egress-compose-file': value.egressComposeFile };
...
test('production and path scope escapes are rejected before Docker', async () => {
  ...
  let calls = 0;
  try {
    await assert.rejects(runSingletonAction({ ...ARGS, project: 'booking-prod', 'compose-file': value.composeFile, 'egress-compose-file': value.egressComposeFile }, {
      releaseRoot: value.releaseRoot, dockerExecutable: '/trusted/docker', commandRunner: async () => { calls += 1; return ok(); },
    }), /scope is invalid/);
    await assert.rejects(runSingletonAction({ ...ARGS, 'compose-file': 'relative.yml', 'egress-compose-file': value.egressComposeFile }, {
      releaseRoot: value.releaseRoot, dockerExecutable: '/trusted/docker', commandRunner: async () => { calls += 1; return ok(); },
    }), /must be absolute/);
