...
Before and after the scan, the generator independently runs Docker image inspection and requires the exact repository digest, `org.opencontainers.image.revision` Git SHA, `uk.happybooking.component` label, and stable image ID/repository-digest set. The Docker-pinned Syft report must independently bind the exact repository name, requested `repository@sha256` input, repository digest, Docker image ID, and a self-consistent native manifest identity. The native Docker manifest identity is deliberately not equated with the registry repository digest because Docker may canonicalize them differently. The report descriptor must prove squashed scope, all-file selection, and SHA-256 as its only file hasher. Multiple installed occurrences with the exact same package name, version, and PURL collapse to one canonical package while the native Syft report preserves their locations. It fails closed on a wrong scanner/schema version, absent package identity/PURL, absent/duplicate/non-absolute file path, missing/multiple/non-lowercase SHA-256 digest, existing output, or any scan-time image drift.
