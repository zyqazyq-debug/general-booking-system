...
async function waitForOpenApi(child) {
  ...
  for (let attempt = 0; attempt < 60; attempt += 1) {
    if (child.exitCode !== null) {
      throw new Error(`backend exited before OpenAPI was ready (exit=${child.exitCode})`);
    }
    try {
      const response = await fetch(openapiUrl, { signal: AbortSignal.timeout(1000) });
      if (response.ok) {
...
function resolveGeneratorCli() {
  ...
  const packageJson = path.resolve(path.dirname(runtimeEntrypoint), '../package.json');
...
async function run() {
  ...
  try {
    await waitForOpenApi(child);
    const cli = resolveGeneratorCli();
    const generator = spawn(process.execPath, [cli, openapiUrl, '-o', generatedClient], {
      cwd: repoRoot,
      env: process.env,
      stdio: 'inherit',
    });
