...
function findFileByContent(keyword) {
    ...
    for (const folder of historyFolders) {
        ...
        if (fs.existsSync(entriesPath)) {
            try {
                ...
                if (entriesData && entriesData.resource && entriesData.resource.includes('user.ts')) {
    ...
    if (best) {
        ...
        fs.writeFileSync('D:\\用户目录\\桌面\\编程\\通用预约系统\\frontend\\src\\domains\\user\\api\\user.ts', content);
        console.log('Restored to domains/user/api/user.ts');
    } else {
        console.log(`Not found: ${keyword}`);
    }
}

findFileByContent('loginTelegramApi');
