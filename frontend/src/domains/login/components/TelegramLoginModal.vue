    ...
    <view class="modal-body text-center">
        ...
        <view id="telegram-login-container" class="telegram-container">
          <view v-if="loadingWidget" class="loading-spinner">
            ...
            <text class="loading-text">{{
              t('login.telegram_widget_loading')
            }}</text>
          </view>
        </view>
        <text class="text-muted mt-2">{{
          t('login.telegram_widget_tap_hint')
        }}</text>
        <button class="telegram-action-deep-link mt-2" @click="openTelegramApp">
      ...
      <view v-if="showFallback" class="fallback-section">
        ...
        <text class="fallback-title">{{
          t('login.telegram_widget_fail_title')
        }}</text>
        <text class="fallback-desc">{{
          t('login.telegram_widget_fail_desc')
        }}</text>
...
const TELEGRAM_LOGIN_STORAGE_KEY = 'booking.telegram.login-ticket.v1';

const persistLoginTicket = () => {
  if (!loginTicket.value || !botUrl.value) return;
  sessionStorage.setItem(
    TELEGRAM_LOGIN_STORAGE_KEY,
    JSON.stringify({
      ticketId: loginTicket.value,
      botUrl: botUrl.value,
...
const clearPersistedLoginTicket = () => {
  sessionStorage.removeItem(TELEGRAM_LOGIN_STORAGE_KEY);
};

const restoreLoginTicket = () => {
  const raw = sessionStorage.getItem(TELEGRAM_LOGIN_STORAGE_KEY);
  if (!raw) return false;
  try {
    const saved = JSON.parse(raw) as { ticketId?: unknown; botUrl?: unknown };
    if (
      typeof saved.ticketId !== 'string' ||
      !saved.ticketId ||
      typeof saved.botUrl !== 'string' ||
      !/^https:\/\/(t\.me|telegram\.me)\//.test(saved.botUrl)
    ) {
      clearPersistedLoginTicket();
      return false;
    }
    loginTicket.value = saved.ticketId;
    botUrl.value = saved.botUrl;
    return true;
  } catch {
    clearPersistedLoginTicket();
    return false;
...
const openTelegramApp = async () => {
  try {
    ...
    // Open deep link
    const opened = window.open(botUrl.value, '_blank');
    if (opened) opened.opener = null;
    if (!opened) {
      // Embedded browsers can block popups even for a direct user
      // gesture. A same-tab Telegram deep link is the reliable fallback.
      window.location.assign(botUrl.value);
    }
  ...
  const poll = async () => {
    ...
    try {
      ...
      if (res.status === 'success' && res.access_token) {
        ...
        emit('update:visible', false);
        uni.showToast({
          title: t('login.toast.login_success'),
          icon: 'success',
        });
        ...
        clearPersistedLoginTicket();
        uni.showToast({
          title: t('login.telegram_link_expired'),
          icon: 'none',
        });
      } else {
...
onMounted(() => {
  // A popup-blocked browser may have navigated to Telegram in the same tab.
  // Resume the original ticket when the user returns instead of losing the
  // only polling context during navigation.
  if (restoreLoginTicket()) startPolling();
  if (props.visible) {
    setTimeout(initTelegramWidget, 100);
  }
});
