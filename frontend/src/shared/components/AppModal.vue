<template>
  <view v-if="visible" class="app-modal-mask" role="dialog" aria-modal="true" :aria-label="title" @click="close">
    <view 
        class="app-modal-content" 
        :class="[`size-${size}`, `variant-${variant}`, { 'center-title': centerTitle }]"
        @click.stop
    >
        <view class="modal-header">
            <text class="modal-title" role="heading" aria-level="1">{{ title }}</text>
            <view class="close-action" role="button" aria-label="关闭" @tap.stop="close">×</view>
        </view>
        <scroll-view v-if="scrollable" scroll-y class="modal-body-scroll" @scroll="onScroll" @scrolltolower="onScrollToLower">
        ...
        <view v-if="showScrollHintState && showScrollHint" class="scroll-hint" @click.stop>
...
.app-modal-content {
    ...
    &.size-sm { width: 520rpx; }
    &.size-md { width: 600rpx; }
    &.size-lg { width: 680rpx; }
    &.size-fullscreen { 
        width: 100vw; 
        height: 100vh; 
        max-width: 100vw; 
    ...
    &.size-custom { width: v-bind(width); }
    &.variant-workspace {
