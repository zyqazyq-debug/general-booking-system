...
import { initializeUserSessionEffects, loginWithTelegramWebApp } from '@/core/auth/user-session-effects';
...
const showDebugConsole = computed(() => {
  ...
  if (typeof window !== 'undefined' && /debugConsole=1/.test(window.location.href)) return true;
  return uni.getStorageSync('debug_console') === '1';
});
const shouldLog = computed(() => {
  if (!import.meta.env.DEV) return false;
  if (typeof window !== 'undefined' && /debugLog=1/.test(window.location.href)) return true;
...
onLaunch(async () => {
  ...
  bootstrapAppLaunch(userStore as any, {
    loginWithTelegramWebApp: (token) => loginWithTelegramWebApp(userStore as any, token),
  });
  ...
  if (tg && isTelegramWebAppRuntime()) {
    ...
    if (shouldLog.value) {
      console.log('[App] Checking Telegram WebApp initDataUnsafe:', tg.initDataUnsafe);
    ...
    if (startParam && startParam.startsWith('ic_')) {
      ...
      if (slug) {
        ...
        setTimeout(() => {
          if (shouldLog.value) console.log('[App] Redirecting to booking detail:', slug);
          uni.reLaunch({
