...
function findFileByContent(keyword) {
    ...
    for (const folder of historyFolders) {
        ...
        if (fs.existsSync(entriesPath)) {
            try {
                ...
                if (entriesData && entriesData.resource && entriesData.resource.includes('share/view.vue')) {
    ...
    if (best) {
        ...
        fs.writeFileSync('D:\\用户目录\\桌面\\编程\\通用预约系统\\frontend\\src\\pages\\share\\view.vue', content);
        console.log('Restored view.vue');
    } else {
        console.log(`Not found: ${keyword}`);
    }
}

findFileByContent('share/view.vue');
